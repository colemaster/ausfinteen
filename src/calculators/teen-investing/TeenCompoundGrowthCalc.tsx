    ...
    <Card variant="glass" className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
        ...
        <StatCard
          ...
          value={`$${Math.round(fvBank30).toLocaleString()}`}
          color="amber"
          subtext="At 4.5% interest rate"
        />
        <StatCard
          label="ASX ETF Portfolio at Age 30"
          value={`$${Math.round(fvEtf30).toLocaleString()}`}
          color="green"
          subtext={`+$${Math.round(etfCompoundInterest).toLocaleString()} in free compound growth!`}
        />
      </div>
